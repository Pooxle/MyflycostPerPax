// ==UserScript==
// @name         Better Myflyclub
// @namespace    http://tampermonkey.net/
// @version      0.1.1
// @description  try to take over the world!
// @author       Aphix/Torus (original cost per PAX by Alrianne), mdons (original creator of this mod), Pooxle (Updated for Myflyclub)
...
(function () {
	...
	async function loadCompetitionForLink(airlineId, link) {
		...
		if ($("#linkCompetitons .data-row").length == 0) {
			$("#linkCompetitons").append(
				"<div class='table-row data-row'><div style='display: table-cell;'>-</div><div style='display: table-cell;'>-</div><div style='display: table-cell;'>-</div><div style='display: table-cell;'>-</div><div style='display: table-cell;'>-</div><div style='display: table-cell;'>-</div></div>"
			);
		}
	...
	async function loadLink(airlineId, linkId) {
		...
		const [linkCompetition, linkHistory] = await Promise.all([loadCompetitionForLink(airlineId, link), loadHistoryForLink(airlineId, linkId, cycleCount, link)]);
    ...
    function PriceAdjustRatio(Pricesensitivitymodifier) {
        let Deltafromstandardprice = Price - standarPrice;

        return 1-0.05+Deltafromstandardprice*Pricesensitivitymodifier/standardPrice
    }
	...
	function calcFlightTime(plane, distance) {
        ...
        if (plane.airplaneType.toUpperCase() == "PROP") {
            let a = min(distance, 250);
            let b = min(max(0, distance - a), 350);
            let d = max(0, distance - (a + b));
            let time_flight = a / min(speed, 400) + b / min(speed, 525) + d / speed;
	...
	function calcFuelBurn(plane, distance) {
        ...
        if (plane.airplaneType.toUpperCase() == "HELICOPTER"){
            Ascendtime = 0
        }
		if (timeFlight > Ascendtime) {
			return ((Math.min(Ascendtime,timeFlight) * plane.fuelBurn * 5.25 + (max(0,timeFlight - Ascendtime)) * plane.fuelBurn));
		} else {
			return ((Math.min(Ascendtime,timeFlight) * plane.fuelBurn * 5.25 + (max(0,timeFlight - Ascendtime)) * plane.fuelBurn));
		}
	}

    function calcmodelFuelBurn(model, duration){
        let timeFlight = duration;
        let Ascendtime = Math.min(50,duration / 3 * 2);
        if (model.airplaneType.toUpperCase() == "PROP") {
            Ascendtime = 18
        }
        if (model.airplaneType.toUpperCase() == "HELICOPTER"){
            Ascendtime = 0
        }
		return ((Math.min(Ascendtime,timeFlight) * model.fuelBurn * 5.25 + (Math.max(0,timeFlight - Ascendtime)) * model.fuelBurn));
		}
	...
	window.updateAirplaneModelTable = function (sortProperty, sortOrder) {
        ...
        let min_quality = parseInt($("#min_quality").val(), 10);
        let max_Price = parseInt($("#max_Price").val(), 10);
        let Targetquality = parseInt($("#targetQualityCost").val(), 10);
		...
		for (let plane of loadedModelsOwnerInfo) {
			...
			let maintenance = plane.capacity * 120 * 0.52;
            let distancefactor = (0.5 + 0.012 * Math.pow(flightDuration/60,2));

			let Slotfee = (64 * Multiplier);
            let LandingFee = (plane.capacity * Multiplier );
            let airport_fee = (Slotfee + Slotfee + LandingFee + LandingFee) * 2 ;
			let crew_cost = plane.capacity * (flightDuration / 60) * 9 + (Math.pow(100/34,2.1) * plane.capacity*(flightDuration / 60)) ;
			let inflight_cost = ( -2 * (flightDuration / 60)) * plane.capacity * 2 ;
			...
			plane.fuel_total = (plane.fbpf * 0.08 * distancefactor + airport_fee +crew_cost + inflight_cost) * plane.max_rotation + depreciationRate + maintenance;
			//plane.cpp = airport_fee
		...
		$.each(loadedModelsOwnerInfo, function (index, modelOwnerInfo) {
			...
			if (modelOwnerInfo.cpp === -1 || modelOwnerInfo.capacity < min_capacity || modelOwnerInfo.quality < min_quality || modelOwnerInfo.price > max_Price|| (modelOwnerInfo.in_use < min_circulation && !isOwned) || (owned_only && !isOwned)) {
				return;
			}
            ...
            row.append("<div class='cell' align='right'>" + modelOwnerInfo.quality );
			row.append("<div class='cell' align='right'>" + modelOwnerInfo.range + " km</div>");
			row.append("<div class='cell' align='right'>" + modelOwnerInfo.fuelPerPax);
			...
			row.append("<div class='cell' style='" + (modelOwnerInfo.in_use >= 0 ? "text-shadow: gold 0px 0px 3px;" : "") + "'  align='right'>" + modelOwnerInfo.in_use + "</div>");
	...
	const columnWidthPercents = [14, 9, 9, 6, 5, 7, 7, 9, 7, 6, 6, 4, 4, 3];
    ...
    //$("#airplaneModelSortHeader").append(
		//#'<div class="cell clickable" data-sort-property="max_rotation" data-sort-order="ascending" onclick="toggleAirplaneModelTableSortOrder($(this))" align="right">Owned</div>'
	//);
	$("#airplaneModelSortHeader").append(
...
`);
	...
	$("#airplaneCanvas .mainPanel .table").append(`
    <div class="cell detailsSelection">Distance: <input type="text" id="fightRange" value="1000" /></div>
    <div class="cell detailsSelection">Runway length: <input type="text" id="runway" value="4000" /></div>
    <div class="cell detailsSelection">Min. Capacity: <input type="text" id="min_capacity" value="0" /></div>
    <div class="cell detailsSelection">Min. Circulation: <input type="text" id="min_circulation" value="-1" /></div>
    <div class="cell detailsSelection">Min. Pax Experience: <input type="text" id="min_quality" value="0" /></div>
    <div class="cell detailsSelection">Min. Pax Experience: <input type="text" id="max_Price" value="605600000" /></div>
...
`);
	...
	var newDataFilterElements = ["#fightRange", "#runway", "#min_capacity", "#min_circulation", "#owned_only", "#use_flight_total", "#min_quality","#max_Price"];
	...
	window.updateModelInfo = function (modelId,airline, index) {
		if (_modelId != modelId) {
		...
		let baseSlotFeeFrom = 64;
        let baseSlotFeeTo = 64;
		...
		for (let row of $(".frequencyDetail .airplaneRow")) {
			...
			maintenance += model.capacity * 120 * utilisation ;
        ...
        let duration = linkModel.duration;
        let fuelburn = calcmodelFuelBurn(model,duration);
        let distancefactor = (0.5 + 0.012 * Math.pow(linkModel.duration/60,2));

		let fuelCost = fuelburn * 0.08 * frequency * distancefactor ;
		//let fuelCost = duration;
        let slotfee = (baseSlotFeeFrom + baseSlotFeeTo) * plane_category * frequency;
        let landingfee = (model.capacity * airportTo.size + model.capacity * airportFrom.size ) * frequency;

		let crewCost = (Math.pow(100/34,2.1) * model.capacity * durationInHour + 9 * model.capacity * durationInHour) * frequency;
		let airportFees = (slotfee + landingfee);
		let servicesCost = (serviceLevelCost * durationInHour) * model.capacity * frequency;
		let cost = fuelCost + crewCost + airportFees + depreciationRate + servicesCost + maintenance;
        let cost1 = fuelCost + crewCost*0.6 + airportFees + depreciationRate + servicesCost*0.6 + maintenance;
        let cost2 = fuelCost + crewCost*0.5 + airportFees + depreciationRate + servicesCost*0.5+ maintenance;
        ...
        $("#airplaneModelDetails #ppd").text("$" + commaSeparateNumber(Math.floor(cost1 / (model.capacity/2.5 * frequency))) + " * " + Math.floor(model.capacity/2.5 * frequency));
        $("#airplaneModelDetails #cpbp").text("$" + commaSeparateNumber(Math.floor(cost2 / (model.capacity/6 * frequency))) + " * " + Math.floor(model.capacity/6 * frequency));
		$("#airplaneModelDetails #cps").text( (Math.floor(duration)) );
	};
...
<div class="table-row">
    <div class="label">
        <h5>Cost per Economy PAX:</h5>
    </div>
    <div class="value" id="cpp"></div>
</div>
<div class="table-row">
    <div class="label">
        <h5>Cost Per Buisness PAX:</h5>
    </div>
    <div class="value" id="ppd"></div>
</div>
<div class="table-row">
    <div class="label">
        <h5>Cost Per First Class PAX:</h5>
    </div>
    <div class="value" id="cpbp"></div>
</div>
<div class="table-row">
    <div class="label">
